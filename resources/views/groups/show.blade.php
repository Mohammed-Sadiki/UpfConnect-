<x-app-layout>
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        {{-- Header du groupe --}}
        <div class="bg-white overflow-hidden shadow-lg mb-6 border border-gray-200">
            {{-- Cover --}}
            ...
            <div class="px-6 pb-6">
                <div class="flex flex-col sm:flex-row sm:items-end -mt-12 sm:-mt-16">
                    ...
                    <div class="mt-4 sm:mt-0 sm:ml-6 flex-1">
                        ...
                        <p class="mt-2 text-gray-600 max-w-2xl">{{ $group->description }}</p>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if($isMember)
                    <div class="bg-white p-6 shadow-md border border-gray-200">
                        <h3 class="text-sm font-semibold text-gray-900 mb-4">Publier dans le groupe</h3>
                        <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data" x-data="{ imagePreview: null }">
                            ...
                            <div class="mb-4">
                                <textarea name="content" rows="3" placeholder="Partagez quelque chose avec le groupe..." class="w-full rounded-lg border-gray-300 focus:border-cyan-500 focus:ring-cyan-500 resize-none" required></textarea>
                            </div>

                            {{-- Image Preview --}}
                            <div x-show="imagePreview" class="mb-4 relative">
                                <img :src="imagePreview" class="max-h-48 rounded-lg">
                                <button type="button" @click="imagePreview = null; $refs.imageInput.value = ''" class="absolute top-2 right-2 w-8 h-8 bg-gray-800 text-white rounded-full flex items-center justify-center hover:bg-gray-700">
                            ...
                            <div class="flex items-center justify-between">
                                <label class="flex items-center px-4 py-2 text-gray-600 hover:text-cyan-600 cursor-pointer transition-colors">
                ...
                @elseif(!$isMember && !$hasPendingRequest)
                    <div class="bg-gray-50 rounded-xl p-6 text-center border border-gray-200">
                        <p class="text-gray-600">Rejoignez ce groupe pour publier et interagir avec les membres.</p>
                    </div>
                @endif
                ...
                @else
                    <div class="bg-white p-12 text-center shadow-md border border-gray-200">
                        ...
                        <p class="mt-2 text-gray-500">Soyez le premier à publier dans ce groupe !</p>
                    </div>
                @endif
            </div>
                        ...
                        @foreach($members as $member)
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('profile.show', $member) }}" class="text-sm font-medium text-gray-900 hover:text-cyan-600 truncate">{{ $member->name }}</a>
