    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Messagerie</h1>
                    <p class="text-xs text-gray-500">Vos conversations</p>
                </div>
            </div>
        </div>
        ...
        @if($messages->count() > 0)
            <div class="bg-white shadow-md overflow-hidden border border-gray-200">
                ...
                <a href="{{ route('messages.show', $other) }}"
                   class="flex items-center p-4 border-b border-gray-200 last:border-0 transition-all duration-200 hover:bg-gray-50
                    ...
                    <div class="relative flex-shrink-0">
                        @if($other->avatar)
                            <img src="{{ Storage::url($other->avatar) }}"
                                 alt="{{ $other->name }}"
                                 class="w-13 h-13 w-12 h-12 rounded-full object-cover ring-2 ring-gray-200">
                        @else
                            <div class="w-12 h-12 rounded-full flex items-center justify-center ring-2 ring-gray-200"
                                 style="background:linear-gradient(135deg,#0ea5e9,#8b5cf6)">
                        ...
                        @endif
                        <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 {{ $other->isOnline() ? 'bg-emerald-500' : 'bg-gray-300' }} rounded-full border-2 border-white" title="{{ $other->isOnline() ? 'En ligne' : 'Hors ligne' }}"></span>
                    </div>

                    {{-- Infos --}}
                    <div class="ml-4 flex-1 min-w-0">
                        <div class="flex justify-between items-baseline mb-0.5">
                            <h3 class="font-semibold text-gray-900 {{ $isUnread ? 'font-bold' : '' }} truncate">{{ $other->name }}</h3>
                            <span class="text-xs text-gray-400 whitespace-nowrap ml-2">{{ $msg->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-sm truncate {{ $isUnread ? 'text-gray-700 font-medium' : 'text-gray-500' }}">
                        ...
                        @if($other->department)
                            <p class="text-xs text-gray-400 mt-0.5">{{ $other->department }}</p>
                        @endif
                    </div>
        ...
        @else
            {{-- État vide --}}
            <div class="bg-white shadow-md p-14 text-center border border-gray-200">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-2">Aucune conversation</h3>
                <p class="text-gray-500 text-sm mb-6">Commencez à discuter avec vos connexions</p>
