<x-app-layout>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Événements universitaires</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition border border-gray-200">
            <div class="relative h-40 bg-gray-100">
                ...
                @endif
                <div class="absolute right-3 top-3 rounded-lg bg-white/90 backdrop-blur-md px-3 py-1 text-center shadow">
                    <span class="block text-xs font-bold text-red-500 uppercase">{{ $event->event_date->translatedFormat('M') }}</span>
                    <span class="block text-lg font-bold leading-none text-gray-900">{{ $event->event_date->format('d') }}</span>
                </div>
            </div>
            <div class="p-5">
                <a href="{{ route('events.show', $event) }}" class="text-lg font-bold text-gray-900 hover:text-blue-600">{{ $event->title }}</a>
