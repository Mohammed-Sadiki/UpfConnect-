<x-app-layout>
    <div class="mx-auto max-w-lg px-4 pb-12 pt-2 sm:px-6">
        <div class="mb-6 flex items-center justify-between gap-4">
            <h1 class="text-2xl font-bold tracking-tight text-slate-900" style="color: #0f172a;">Modifier l’événement</h1>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-md">
            <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input id="event_date" type="datetime-local" name="event_date" value="{{ old('event_date', $event->event_date->format('Y-m-d\TH:i')) }}" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" required
                        class="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
                    @error('description')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="image" class="mb-1 block text-sm font-semibold text-slate-800">Nouvelle image (optionnel)</label>
                    @if($event->image && !\Illuminate\Support\Str::startsWith($event->image, 'http'))
                        <p class="mt-1 text-xs text-slate-500">Image actuelle conservée si vous ne choisissez pas de fichier.</p>
                    @endif
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="flex-1 rounded-xl bg-blue-600 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-blue-700">Enregistrer</button>
                    <a href="{{ route('events.show', $event) }}" class="inline-flex flex-1 items-center justify-center rounded-xl border border-slate-300 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Annuler</a>
                </div>
            </form>
        </div>
    </div>
